...
| **Summary** | 1 per actor | header band: avatar, name, type badge, `quote` · `summary` paragraph · three columns **Enduring traits / In context: <title> / When traits meet context**, each with a caption naming its source (§13), from `summarySlots` · "→ see appendix" marker on any truncated column · "+N more contexts → appendix" when applicable · **full lists in speaker notes** | **never paginates, never shrinks.** Fits as many capped items as the column holds — as many as fit, 5 down to 1, minimum 1 — and records a warning |
